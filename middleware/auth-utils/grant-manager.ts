...
const getProtocol = (opts): { request: any } => {
...
const postOptions = (manager, path?: string) => {
...
const fetch = (manager, handler, options: http.ClientRequestArgs, params) => {
...
class GrantManager {
   ...
   */
  public obtainDirectly(username, password, callback, scopeParam?: string) {
    const params = {
      username,
      password,
   ...
   * @param {Function} callback Optional callback, if not using promises.
   */
  public obtainFromCode(ctx, code, sessionId, sessionHost, callback) {
    const params = {
      code,
   ...
   */
  public obtainFromClientCredentials(callback, scopeParam?: string) {
    const params = {
   ...
   */
  public ensureFreshness(grant, callback?: () => void) {
    if (!grant.isExpired()) {
  ...
  public userInfo(token, callback) {
  ...
  public isGrantRefreshable(grant) {
